...
8. **L4 human-approval gate (the headline), with an L5 kill-switch.** Held until a
   ...
   is validated — the only valid values are 4 (default) and 5; anything else (below
   L4 or above L5) is **rejected at build**, so a payment can never sit below the
   approval gate nor carry an off-ladder level.
