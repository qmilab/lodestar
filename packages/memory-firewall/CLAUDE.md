...
- **Evidence aggregators** (`src/stores/evidence-store.ts`) — two pure scalars over an `EvidenceSet`, sharing one independence-group helper (`strongestPerGroup`) so they can't drift on what counts as corroboration:
  - `aggregateStrength` — the **gate input**, normalized `(S − C)/(S + C) ∈ [−1, 1]`. `cognitive-core` adopts at `> 0` and auto-promotes to `supported` at `>= 0.7` (plus the Parallax quality gate). An all-supporting set is **always exactly `1.0`** regardless of source count — deliberate, to keep the promotion threshold calibration-stable.
  - `corroborationStrength` (#158, ADR-0036) — a **non-gate** ranking/legibility scalar in `[0, 1)` that *rises* with the number of **independent** supporting groups (noisy-OR, quality-weighted, saturating; a `0.95` per-source ceiling keeps even a lone `direct_observation` below the cap so corroboration is legible). It feeds **no** threshold, so adding it shifts no belief's lifecycle and Parallax is untouched. The harvest projection surfaces it as `MemoryCandidate.corroboration` ("best-evidenced first"). A v0 heuristic like its sibling — both **off** the stable public-API ledger.
