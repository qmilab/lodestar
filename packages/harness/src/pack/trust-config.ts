...
/** An empty trust config: no pinned keys, so a signed pack is rejected unless an author is pinned elsewhere (e.g. a `--author-key` flag), no badge attester is trusted, and no index publisher is trusted. */
function emptyTrustConfig(): PackTrustConfig {
  return { author_keys: [], attester_keys: [], index_publisher_keys: [] }
}
