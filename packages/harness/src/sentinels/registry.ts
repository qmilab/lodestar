 ...
 *
 * Null-prototype, not a plain `{ ... }` literal: a probe-pack manifest is
 * untrusted (loader invariant 3) and a sentinel id is validated only as
 * kebab-case. The id `constructor` passes that regex and, on an
 * ordinary object, would resolve to the inherited
 * `Object.prototype.constructor` — a truthy non-Sentinel that slips past a
 * lookup's existence check and later crashes the runner. With no prototype,
 * any id that is not an own key resolves to `undefined`, so an unknown id is
 * always rejected. Do NOT switch this back to a plain object literal.
 */
export const FIRST_PARTY_SENTINELS: Readonly<Record<string, SentinelFactory>> = Object.freeze(
  Object.assign(Object.create(null) as Record<string, SentinelFactory>, {
