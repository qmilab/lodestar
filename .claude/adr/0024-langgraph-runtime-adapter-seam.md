...
- **Related:** ADR-0002, ADR-0003 (proxy decision synthesis), ADR-0004 (TS-level
  ...
  ADR-0015 (signed approval side-channel), epic #75, #83, `packages/guard-mcp/`,
...
boundary, (b) cross the language barrier to the TS engine, (c) preserve the
"no work before approval" invariant across that barrier, and (d) surface an L4
hold in the framework's own idiom.
...
- **Enforcement must sit where it controls whether the tool body runs** — i.e.
  wrapping the tool (or the prebuilt `ToolNode`), not observing it.
- **LangGraph has a first-class human-in-the-loop primitive**: `interrupt()`
  pauses the graph at a checkpoint and `Command(resume=…)` continues it. An L4
  hold maps onto this natively — no custom approval UI.
...
and the approval-polling / Ed25519 hold path the MCP proxy already runs. The only
genuinely new code is the RPC protocol + gate server (TS) and the hook (Python).

### 3. The seam: wrap tools for two-phase; `interrupt` for L4 holds

Callbacks are rejected for enforcement (observe-only). The hook wraps each
governed tool (and/or the prebuilt `ToolNode`) so the tool body runs **only**
inside the engine's execute phase. A held L4 surfaces as a LangGraph
`interrupt` carrying the approval request (action id, request id, deadline); the
graph pauses at a checkpoint, the resolution arrives out-of-band through the
existing signed side-channel (`lodestar approve`, ADR-0010 / ADR-0015), and on
`Command(resume=…)` the hook re-checks with the sidecar and either proceeds to
execute or returns a soft denial. For headless runs the hook may instead
block-poll up to the deadline exactly as the proxy does (`approval_timeout_ms`);
`interrupt` is the idiomatic default.

### 4. Two-phase preserved by remoting execution back into Python ("hook as downstream")
...
decision from `observedBeliefIds()`), and on a hold polls for approval up to the
deadline. **Only if allowed** does `kernel.execute()` fire — and the governed
tool the kernel executes is registered so its `execute()` is an RPC call **back**
to the Python hook ("now run tool X with args Y"). The hook runs the real Python
...
This is the proxy's model verbatim — **from the kernel's perspective the Python
hook is just another downstream**, so the kernel, the gate, the cognitive core,
and the arbiter are untouched (no schema change, no kernel change). It guarantees
the CLAUDE.md invariant "tools that do work before approval are bugs" *across the
language boundary*: the only place the tool body runs is inside the TS execute
phase, which is reached only after the gate (and any hold) clears. The cost is a
re-entrant RPC (Python → TS gate → TS execute → Python run → TS observe → Python
return); the transport is bidirectional, so this is well-defined.

### 5. Transport: bidirectional newline-delimited JSON-RPC over stdio
...
dependencies, the same framing MCP itself uses, and it supports the bidirectional
requests the remoted-execute callback needs. ("Connect to an already-running
sidecar over a socket" is a deferred mode.) Operator-pinned configuration (the
signed policy document, approver keys, sentinel ids, persistence) lives **only**
in the sidecar's `RuntimeGateConfig` — a near-twin of `ProxyConfig`; the hook
never holds credentials or policy. This is a **TS/process-level governance
boundary over the agent's actions, not OS containment of the Python process** —
the same honesty caveat as the native adapters (ADR-0004): it governs what the
agent *does*, it does not sandbox what the agent *is*.

### 6. Repository layout: TS stays in `packages/`, Python is a clearly-marked sibling

- **`packages/runtime-core/` (`@qmilab/lodestar-runtime-core`)** — the new TS
  package: the RPC protocol types + the gate server that wraps the engine.
  Reused by every runtime adapter. `lodestar runtime gate` is the CLI.
...
### 7. Locking probes: an always-on TS contract + a runtime-gated end-to-end
...
- **`runtime-gate-enforces-two-phase`** (always-on, `packs/lodestar-core/`):
  ...
  and pins the governance contract the hook relies on — a held L4 touches nothing
  and stays held until a *signed* approval resolves it; an `external_document`
  tool result cannot self-promote a belief; a synthesized decision links the
  observed-belief set; a denied action never reaches the (remoted) execute phase.
  No Python needed, so the core invariant is enforced on every run.
- **`langgraph-tool-calls-are-governed`** (runtime-gated, end-to-end): drives a
  **real Python LangGraph loop** through the hook + sidecar, and **skips loudly**
  (exit 0, banner) when Python/LangGraph is absent — mirroring the DB-gated
  `tool-poisoning-cross-session` and the sandbox-gated
...
- **The epic gets a shared spine.** One TS sidecar + RPC protocol; CrewAI (#84)
  ...
  expensive, risky work is done once.
...
- **The engine is untouched.** No new core schema, no kernel change — the hook is
  "just another downstream." New surface is the RPC protocol, the gate server,
  and the Python hook.
- **The re-entrant remoted-execute RPC is the main new mechanic** and the thing
  most likely to surprise (bidirectional framing, error/timeout propagation
  across the boundary, async tools). It preserves two-phase faithfully; it should
  be de-risked with a thin end-to-end spike before the full slice is built.
- **Holds reuse the framework's own idiom** (`interrupt`) and the existing signed
  side-channel — no new approval UI, no new forgery surface.
- **Concurrency is already safe.** The proxy's grow-only `observedBeliefIds` set
  is concurrency-safe by construction (ADR-0003); parallel LangGraph tool calls
  inherit that posture (they over-link, the safe direction).
