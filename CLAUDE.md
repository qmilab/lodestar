...
**Status**: v0.5.0 published to npm (all 27 packages) and PyPI (the four
`lodestar-runtime-*` Python hooks) via CI trusted publishing — staged
publishing, maintainer-approved; v0.2 architecture locked. The 0.5.0
release added the 26th and 27th npm packages —
`@qmilab/lodestar-adapter-vector` (the governed Vector/RAG retrieval adapter
over pgvector, ADR-0039) and `@qmilab/lodestar-adapter-payments` (the
governed payment-send adapter, ADR-0040) — alongside the cognitive-core
belief-enrichment epic (#154, ADRs 0031–0037), the interop/hygiene tail of
#140 (the stable `firewall.*@1` event contract, the public-API stability
ledger + its probe), and the `adapter-sql` server-side-cursor bounded fetch
(#101, ADR-0038). The 0.4.0 release added the 25th npm package,
`@qmilab/lodestar-runtime-core` (the language-agnostic governance-gate
sidecar, ADR-0024/ADR-0025), and the non-MCP runtime-adapter epic's PyPI
hooks — `lodestar-runtime-client` (the shared RPC client) plus
`lodestar-langgraph`, `lodestar-crewai`, and `lodestar-autogen`
(ADR-0026/ADR-0027/ADR-0028) — alongside the trust-pack registry epic (#76)
and probe-runner scoped-env + OS-sandbox execution (ADR-0022/ADR-0023). The
two packages that had landed at 0.3.0 were the 23rd,
`@qmilab/lodestar-adapter-sql` (the governed SQL/database adapter,
...
format, ADR-0014). Eighty probes pass under
strict TypeScript (three need a Postgres test database; one needs a Python
+ LangGraph runtime; one needs a Python + CrewAI runtime; one needs a
Python + AutoGen runtime — see below).
Seventy-six live in the first-party pack
...
candidacy gate + firewall-authored reconstruction), one cognitive-core
reflection-derive probe (`reflection-derives-supersession-from-conflict` — the
#154 item-B reflection DERIVE rule: a new third reflection rule that *derives* a
contradiction from live belief state — two `supported` beliefs in one scope
sharing a claim's `structured_predicate.(subject, relation)` but asserting
different `object`s — and proposes a `belief_supersession` (older `superseded_by`
newer). **Propose-only:** `run()` surfaces the proposal but never applies it even
under `apply: true` (a derived conflict is a human-adjudicated hypothesis — the
world changed vs. genuine disagreement); a reviewer applies it via the public
`applyProposal` (the `markSuperseded` apply path already existed — only detect is
new). It fires once, in the pass whose window holds the later belief's
`belief.adopted` event (single-fire idempotence), reuses the evidence-linker's
exported `isEligibleJoinPeer` gate + the shared `predicateKey`/`stableStringify`
(single source, no drift) narrowed to `truth_status:supported`, and pairs only
**equal-sensitivity** beliefs — stricter than the linker's `≤` ceiling, because
the output names both beliefs to a human, so a cross-compartment pairing would
signal a higher belief's existence in either direction (the secret belief is
itself a window trigger, the direction `max_sensitivity` alone would not block).
Authenticity is moot — the rule reads everything substantive from the
firewall-governed store and only proposes, so a forged `belief.adopted` event can
at most trigger a scan of an already-legitimate belief. Note the interaction with
the #157 ingest-time join, which *blocks* adopting a second conflicting belief,
so the derive rule's real-world targets are conflicts that entered via different
paths (imports / assertions / merged sessions); the probe seeds them directly.
Sixteen checks A–K incl. propose-only/no-auto-apply, explicit-apply,
quarantined-peer + equal-sensitivity + unverified-peer gates, and single-fire
idempotence; in-memory; ADR-0032 lineage / #154 child B), one cognitive-core
generic-extractor probe (`generic-llm-extractor-stays-unverified` — the #154
item-C-2 opt-in LLM-driven generic claim extractor: `createGenericLLMExtractor(model)`
claims the reserved `__generic__` fallback slot so observation text with no
schema-bound extractor still yields claims, **registered explicitly, never a
built-in** (replay-stable deterministic extraction stays the default). The
provider-agnostic `GenericExtractionModel` seam keeps the LLM client/key/prompt in
the consumer — Lodestar ships none — and every minted claim is `extraction_method:
"llm"`. The partner `GenericAwareEvidenceLinker` stamps that claim's source evidence
at **`model_inference`** quality, so the Round 5 auto-observation (Parallax) gate
keeps the belief `unverified` even at an aggregate strength that would promote a
`direct_observation` claim, and even when a SECOND independent generic observation of
the same predicate corroborates it (Parallax across LLM inferences, like two
`external_document` beliefs). The downgrade lives in the linker **by design** — the
base linker treats `extraction_method` as non-load-bearing, which the #157
cross-belief-join probe relies on — so the opt-in is the extractor *and* its linker,
pinned by a control showing the same `llm` claim via the BASE linker keeps
...
/ epic #154), one cognitive-core world-model-gate probe
(`world-model-withholds-gated-current-state` — the #165 / ADR-0037 consistency
hardening: `CognitiveCore.ingest` step 6 now honours the **same** auto-observation
gate the belief gate does. A claim updates the world model — the **ungated**
"current state" store a planner reads to decide its next action — only if its
evidence both nets positive **and** clears the gate; a positive-but-gated claim
(strongest support `external_document` / `model_inference`) is **withheld** and
recorded on `IngestResult.worldModelWithheld` (hosts carry it as
`world_model_withheld`). Before #165 the write was gated on net-positive strength
alone, so a lone poisoned `external_document` value still wrote current state — a
poisoning **side door**, latent only because nothing reads the world model back yet
(no `planner.ts`), but "a planner reads current state" is the store's whole purpose.
We **withhold** rather than write-and-flag (ADR-0037): the world model has no
read-time gate to enforce a flag, and a flagged write would **shadow** a previously
gate-cleared value — the unverified belief still carries the full record. Four ACs
in-memory: a lone `external_document` claim is withheld (no entry, recorded in
`worldModelWithheld`, belief `unverified`); the gate-cleared `direct_observation`
control DOES write; a gated write never shadows a prior gate-cleared value (read +
history untouched, via an ineligible-peer setup that isolates the gated path); and
the gate keys on **final** evidence, so a corroborated `external_document` claim
clears the gate and writes. Refines ADR-0032's #157 P2#1 world-model rule; no
`packages/core` schema change), and fourteen Policy Kernel probes —
...
`forged-approval-via-http-channel-cannot-execute`), and one **M-of-N quorum**
probe (`quorum-requires-distinct-eligible-approvers` — #175 / ADR-0041: the
threshold is adjudicated by the kernel over each approver's OWN signed
resolution, never attested by a client. If a coordinating collector could gather
N signatures and submit one synthesized grant, the log would record a
*single-approver* decision and "quorum" would degrade into an unverifiable claim
by that intermediary — so **the client accumulates; the kernel adjudicates**. The
load-bearing separation: `approval.granted@1` stays **one approver's vote**,
`approval.quorum_reached@1` is the **authorization** that drives `resolve()`; at
the single-approver threshold the two coincide, which is why that path emits no
quorum event and is byte-identical. Adjudication checks two **orthogonal** things
with two inputs — authenticity (an operator-pinned key signed this exact
resolution) and eligibility (this approver clears the request's
`required_authority`) — because the signed resolution deliberately carries no
authority, so a keys-only adjudicator would let *any* N pinned approvers satisfy
a `secret`-clearance quorum. Twelve cases, ten through a real `guard.wrap()`
session with a real compiled `quorum: 3` policy and real Ed25519 signatures: three
eligible grants satisfy it (three `approval.granted@1` votes + one
`approval.quorum_reached@1` naming all three **with their evidence pointers**, so
a reader re-verifies against their own keys rather than trusting the emitter);
a collector-synthesized single grant cannot satisfy 3 (the headline); M-1 does not
un-park; a duplicate `actor_id` counts once; a deny after M-1 is decisive
regardless of grants collected; a **revoked key's** outstanding vote stops
counting (the verify-time roster — no request-time snapshot, so revocation bites
immediately); an authentically-signed **pinned-but-ineligible** approver is
promoted-but-not-counted (authenticity gates the *log write*, eligibility gates
the *count*); an approver with **no supplied authority record** does not count
(fail closed); the action's own **proposer** cannot count toward its own quorum;
and `quorum` absent is byte-identical to today. Two more drive the **real MCP
proxy** over the real signed `.approvals/` side-channel — the accumulating hold
loop: three votes cast one at a time are each promoted and accumulate **in the
LOG** until the third authorizes the call (the single-slot channel needed no
interface change), and a **partially satisfied** 2-of-3 that hits its deadline is
a **soft denial**, not an approval), one
...
packages/
  ...
  adapters/
    ...
    vector/            # (exists) governed Vector/RAG retrieval adapter (pgvector/Postgres, reuses adapter-sql's SqlConnection): vector.query (L1, untrusted retrieved chunks = external_document — the RAG poisoning surface). Operator-pinned table + namespace allowlist (never agent input); parameterized values (embedding $1::vector / namespace $2 / LIMIT $N bound, only validated-quoted identifiers interpolated); top-k cap bounded server-side by LIMIT; READ ONLY transaction; redacted credentials. Consumer embeds the query (Lodestar ships no model). The no-auto-promote teeth live in cognitive-core's VectorRetrievalExtractor + VectorAwareEvidenceLinker (mirrors doc.read/DocAwareEvidenceLinker): each chunk is external_document so the auto-observation gate keeps it unverified (Parallax across chunks). TS-level boundary, not DB containment; ADR-0039
    payments/          # (exists) governed payment-send adapter: payment.send (L4 egress, irreversible — the strongest human-approval case; operator can pin L5 to disable). Operator-pinned payee allowlist (the exfil guard, enforced in execute = an audited failed action); NEW operator amount ceiling + currency allowlist (rejected at propose via the input-schema superRefine — over-ceiling is never presentable to a human; preconditions don't fire at propose); forwarded idempotency key (no double-charge on replay); operator-fixed endpoint + scoped/redacted credential (resolver seam); strict delivery semantics (an unconfirmed charge fails); no redirect; bounded capture. PaymentProvider seam — Lodestar ships no PSP client/key (generic-HTTP default). Ports messaging's credentials/transport verbatim; no cognitive-core change. TS-level boundary, not payment-network containment; ADR-0040
...
packs/
  lodestar-core/             # (exists, Batch 4) first-party probe pack: 76 probes +
