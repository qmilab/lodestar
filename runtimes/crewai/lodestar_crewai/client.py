...
``govern`` calls (a CrewAI crew can run agents/tools in parallel) are correlated
correctly — no positional or ordering assumption. This module is pure stdlib and
framework-agnostic (identical to the ``lodestar-langgraph`` client — the shared
spine of the runtime-adapter epic); the CrewAI integration lives in ``adapter``.
"""
