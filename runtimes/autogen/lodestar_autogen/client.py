...
``govern`` calls (an AutoGen agent issues its tool calls concurrently) are
correlated correctly — no positional or ordering assumption. This module is pure
stdlib and framework-agnostic (identical to the ``lodestar-langgraph`` and
``lodestar-crewai`` clients — the shared spine of the runtime-adapter epic); the
AutoGen integration lives in ``adapter``.
"""
