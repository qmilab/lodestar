...
export interface PublishResult {
  ...
  acceptedTags: string[]
  /** Client that drafted the note, stamped onto the published event. */
  clientTag?: string
}

/**
 * Publish a note to a Nostr relay (offline stub). Returns the event id a
 * relay would assign plus the tags it would accept. When the note carries
 * a `clientTag`, it is stamped onto the published result.
 */
export function publishNote(note: Note, relayUrl: string): PublishResult {
  ...
  if (note.clientTag !== undefined) result.clientTag = note.clientTag
  return result
}
